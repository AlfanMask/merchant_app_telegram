<script lang="ts">
	...
	import {
		filterMerchantsCategory as filterMerchantsCategoryStore,
	...
	$: {
		...
		let filteredMerchants =
			filterMerchantsCategory != Category.Semua
		...
		if (filterMerchantsFreeParking != Parking.Semua) {
			filteredMerchants = filteredMerchants.filter((o) =>
				filterMerchantsFreeParking == Parking['Bebas Parkir']
					? o.is_parking_free
					: !o.is_parking_free
			);
		}
		...
		if (filterMerchantsOpen != Open.Semua) {
			filteredMerchants = filteredMerchants.filter((o) =>
				filterMerchantsOpen == Open.Open
					? isMerchantOpen(new Date(), o)
					: !isMerchantOpen(new Date(), o)
			);
		}
		...
		displayedMerchants = filteredMerchants.slice(
	...
	const clickMerchantHandler = (id: string) => {
		...
		if (isOpen) {
			...
			isShowRestoClosedMessage =
				'Maaf, resto tutup :( Resto ini buka ' +
				openDays +
				' dari jam ' +
				merchantToOpen.open_hour +
				'.00 sampai jam ' +
				merchantToOpen.close_hour +
				'.00';
		}
...
{#if isComingFromTelegram}
	<div id="bg" class="min-h-screen h-full bg-primary py-8">
		...
		<div id="search" class="fixed px-6 mb-8 flex gap-4 items-center z-50">
			<i class="fa-solid fa-circle-chevron-left text-secondary text-4xl" on:click={clickBackToHome}
			></i>
			<AutocompleteMerchant
				placeholder="Cari resto.."
				inputText={searchInput}
				autocompleteData={allMerchants}
		...
		<div id="main" class="px-4 mt-20">
			...
			<div id="topbar" class="w-full mb-8 flex flex-col gap-4">
				<select
					id="select_category"
					bind:value={filterMerchantsCategory}
					on:change={resetPageIndex}
					class="w-full rounded px-2 py-2 border border-solid border-secondary"
				>
				...
				<div class="grid grid-cols-2 items-center gap-4">
					<select
						id="select_parking"
						bind:value={filterMerchantsFreeParking}
						on:change={resetPageIndex}
						class="rounded px-2 py-2 border border-solid border-secondary"
					>
					...
					<select
						id="select_open"
						bind:value={filterMerchantsOpen}
						on:change={resetPageIndex}
						class="rounded px-2 py-2 border border-solid border-secondary"
					>
		...
		{#if totalPages > 1}
			<div
				id="pagination"
				class="w-full flex justify-between items-center mt-6 opacity-75 fixed z-40 px-6 bottom-[2%] left-1/2 -translate-x-1/2 -translate-y-1/2"
			>
				<i
					class="fa-solid fa-circle-chevron-left text-secondary text-5xl"
					on:click={() => movePage('prev')}
				></i>
				<!-- <span class="!text-lg !font-medium text-dark mx-6 flex items-center">{pageIndex+1}/<span class="!text-sm">{totalPages}</span></span> -->
				<i
					class="fa-solid fa-circle-chevron-right text-secondary text-5xl"
					on:click={() => movePage('next')}
				></i>
			</div>
		{/if}
